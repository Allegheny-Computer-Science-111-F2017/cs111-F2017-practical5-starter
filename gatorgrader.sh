...
if [ "$CHECK" = true ]; then
  ...
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicalfive/ \
                                     --checkfiles RollAndSumDice.java --singlecomments 4 --multicomments 2 --language Java
  determine_exit_code $?
  # --> GatorGrader CHECK: the "new Die()" fragment exists in the code at least three times
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicalfive \
                                     --checkfiles RollAndSumDice.java --fragments "new Die()" --fragmentcounts 3
  determine_exit_code $?
  # --> GatorGrader CHECK: the "int sum" fragment exists in the code at least once
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicalfive \
                                     --checkfiles RollAndSumDice.java --fragments "int sum" --fragmentcounts 1
  determine_exit_code $?
  # --> GatorGrader CHECK: the "println(" fragment exists in the code at least four times
  python3 gatorgrader/gatorgrader.py --nowelcome --directories src/main/java/practicalfive \
                                     --checkfiles RollAndSumDice.java --fragments "println(" --fragmentcounts 4
  determine_exit_code $?
  echo ""
